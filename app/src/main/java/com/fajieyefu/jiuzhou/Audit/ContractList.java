...
import com.fajieyefu.jiuzhou.Query.ContractQuery;
 ...
 */
public class ContractList extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		titleLayout.setTitleText("待确认事项");
		...
		listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (applyInfo.getBill_code().equals("D")) {
					Toast.makeText(ContractList.this, "请等待二次确认", Toast.LENGTH_SHORT).show();
					return;
				}
				Intent intent = new Intent(ContractList.this, ContractQuery.class);
	...
	class MyAsyncTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			String info = WebService.getQueryContractInfo2(account, count);
		...
		protected void onPostExecute(Boolean aBoolean) {
			if (aBoolean) {
				...
				Toast.makeText(ContractList.this, "没有可用数据", Toast.LENGTH_SHORT).show();
			}
